...
use std::{time::Duration, rc::Rc, f32::consts::PI, ops::Add, str::FromStr, path::{Path, PathBuf}};
...
/*impl render::RenderWindow for SDLWindow
{
    fn get_handle(&self) -> &sdl2::video::Window 
    {
...
pub fn main() 
{
    ...
    path_buf.push("axes.pibm");
    repo.load_static_model("axes".to_string(), path_buf.as_path());
    path_buf.pop();
    path_buf.push("rho.pibm");
    repo.load_animated_model("rho_anim".to_string(), path_buf.as_path());
    path_buf.pop();
    path_buf.push("rho.pibs");
    repo.load_armature("rho_armature".to_string(), path_buf.as_path());
    path_buf.pop();
    path_buf.push("clip_idle.piba");
    repo.load_animation("idle".to_string(), path_buf.as_path(), &"rho_armature".to_string());
    path_buf.pop();
    path_buf.push("clip_walk.piba");
    repo.load_animation("walk".to_string(), path_buf.as_path(), &"rho_armature".to_string());
